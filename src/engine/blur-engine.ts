...
function applyMosaic(data: Uint8ClampedArray, width: number, region: Rect, blockSize: number) {
...
export function applyBlurToRegion(
  ...
  if (featherWidth > 0) {
    ...
    applyBlurToRegion(data, canvasWidth, canvasHeight, expandedRegion, method, effectiveBlockSize, 0)
  ...
  switch (method) {
    case 'mosaic':
      applyMosaic(data, canvasWidth, clampedRegion, effectiveBlockSize)
      break
    case 'solid':
...
export function applyBlurToEllipseRegion(
  ...
  applyBlurToRegion(data, canvasWidth, canvasHeight, expandedRegion, method, effectiveBlockSize)
...
export function applyBrushStroke(
  ...
  if (method === 'gaussian') {
    // Use canvas filter for gaussian
    ctx.save()
    ctx.filter = `blur(${Math.round(brushSize / 3)}px)`
    ctx.beginPath()
    const r = brushSize / 2
    for (const p of points) {
