...
export function renderToCanvas(
  ...
  for (const action of actions) {
    applyActionToCtx(ctx, action, canvas.width, canvas.height, displayScale * image.scale)
  }
}
  ...
  switch (action.type) {
    ...
    case 'auto-face': {
      ...
      if (action.method === 'gaussian') {
        ...
        if (shape === 'face') {
          applyBlurToEllipseRegion(imageData.data, canvasW, canvasH, paddedRect, action.method, undefined, angle)
        } else if (shape === 'oval') {
          applyBlurToOvalRegion(imageData.data, canvasW, canvasH, paddedRect, action.method, undefined, angle)
        } else if (angle !== 0) {
          applyBlurToRotatedRectRegion(imageData.data, canvasW, canvasH, paddedRect, action.method, undefined, angle)
        } else {
          const rectFeather = Math.max(3, Math.round(Math.min(paddedRect.width, paddedRect.height) * 0.04))
          applyBlurToRegion(imageData.data, canvasW, canvasH, paddedRect, action.method, undefined, rectFeather)
        }
